...
Roy saying so in the same breath.** The remote Scout first uses RSS/article covers with original URL and publisher attribution, per Roy's September 12 request. FLUX.1 Schnell is a fallback only when no usable cover is supplied, with a hard per-slot inference cap within the shared daily free allocation at its intended volume; it does not enable paid billing. See the remote Scout section in README for the account-wide quota caveat. `local-imagegen` (sibling folder) serves
