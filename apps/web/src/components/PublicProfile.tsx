...
export default function PublicProfile() {
  ...
  async function load(offset = 0) {
    ...
    try {
      ...
      if (!handle) throw new Error('Choose an author from a thread to see their contributions.')
  ...
  return <div className="public-profile">
    ...
    {data && <><h2>Shared work</h2>{data.contributions.length ? <ol>{data.contributions.map(item => <li key={`${item.kind}-${item.id}`}>
      <p className="profile-kicker">{item.kind === 'challenge' ? 'Shared a thread' : item.kind === 'update' ? 'Added a progress update' : 'Joined the discussion'} · <time dateTime={dateISO(item.created_at)} title={formatDateTime(item.created_at)}>{formatDate(item.created_at)}</time></p>
