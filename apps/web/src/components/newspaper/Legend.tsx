...
import { STAGE_ORDER, STAGE_STAMP, STAGE_MEANING, LEGEND_ACTIONS, LEGEND_ACTION_LABEL } from '../../lib/vocab'
import WorldPlate from '../WorldPlate'
import { editionPath } from '../../lib/edition'

const icons = ['plus', 'lightbulb', 'users', 'flask', 'chart-line']
export function Rings({ count, size = 38 }: { count: number; size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true">{Array.from({ length: count }, (_, i) => <circle key={i} cx="24" cy="24" r={21 - i * 3.6} fill="none" stroke="currentColor" strokeWidth="1.5" />)}</svg>
}
export default function Legend({ challenges = [] }: { challenges?: Challenge[] }) {
  ...
  return <div className="np-legend">
    ...
    <section className={'np-legend-rings ' + (section === 'rings' ? 'selected' : '')}>
      <h3>Participation scale</h3><div className="np-rings-scale">{[1,2,3,4,5].map(n => <div key={n}><Rings count={n} /><span>Level {n}</span></div>)}</div><p>More rings, more contributions. Relative activity, not measured impact.</p>
    </section>
