...
export default function CommunityPage({ challenges }: { challenges: Challenge[] }) {
  ...
  return <PagedContent label="Community">{handle ? <PublicProfile /> : <div className="np-community"><h1>People who notice.<br />People who build.</h1><p className="np-deck">Explore the people sharing Challenges in this edition.</p>{authors.map(person => <article key={person.id}><h2><a href={'/people?handle=' + encodeURIComponent(person.handle)}>@{person.handle} →</a></h2><p>{person.location || 'Part of a global conversation'}</p><ul>{challenges.filter(c => c.author.handle === person.handle).map(c => <li key={c.id}><a href={'/c/' + c.slug}>{c.title}</a></li>)}</ul></article>)}{!authors.length && <p>The next story could be yours. <a href="/post">Share a Challenge →</a></p>}</div>}</PagedContent>
}
