...
export default function ChallengePage({ initial }: { initial?: ChallengeData }) {
  ...
  async function refresh() {
    ...
    setData(previous => previous?.challenge.slug === next.challenge.slug ? { ...next, challenge: { ...next.challenge, views_count: Math.max(previous.challenge.views_count, next.challenge.views_count) } } : next); setError(''); document.title = next.challenge.title + ' - Techno Optimists'
  }
  useEffect(() => {
  ...
  return <div className="np-challenge np-challenge-sheet">
    ...
    <div className="np-panel-grid">
      <section className="np-panel np-panel-story" aria-label="Challenge story">
        ...
        <div className="np-story-reading"><PagedContent compact label="Story"><h1>{c.title}</h1><p className="np-deck">{c.summary}</p><p className="np-story-author">Shared by <a href={'/v2/people?handle=' + encodeURIComponent(c.author.handle)}>@{c.author.handle}</a> · {formatDate(c.imported_at || c.created_at)} · <ViewsCount count={c.views_count ?? 0} title={c.title} /></p>{c.body && c.body.split(/\n\s*\n/).map((p, i) => <p key={i}>{p}</p>)}</PagedContent></div>
      </section>
