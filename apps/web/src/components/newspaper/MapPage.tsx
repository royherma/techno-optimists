...
export default function MapPage({ initial }: { initial: Challenge[] }) {
  ...
  return <div className="np-map-page">
    ...
    <p className="np-map-hint">Hover, focus or tap a place to preview its thread below.</p>
    ...
    <article className="np-map-story" aria-label="Selected thread">
      {c ? <>
        ...
        <div className="np-map-copy">
          ...
          <a className="np-arrow-link" href={'/c/' + c.slug}>Explore this thread →</a>
        </div>
      </> : <p>No matching threads have a location yet.</p>}
    </article>
