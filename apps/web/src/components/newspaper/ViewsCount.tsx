...
export default function ViewsCount({ count, title }: { count: number; title: string }) {
  ...
  return <>
    ...
    {open && <PanelDialog title="Challenge views" open={open} onClose={() => setOpen(false)}>
      <h3>{title}</h3><p><strong>{count.toLocaleString('en')}</strong> {count === 1 ? 'view' : 'views'}</p>
      <p>Views count visits to this Challenge, including signed-out readers. Repeat visits from the same browser and connection count once per UTC day.</p>
      <p>Returning on another day adds another view. Browsing past a Challenge in the feed does not count as a view.</p>
    </PanelDialog>}
  </>
}
