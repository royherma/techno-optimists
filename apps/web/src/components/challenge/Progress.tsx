...
export default function Progress({ challenge, updates, me, refresh, panel = false }: { challenge: Challenge; updates: Update[]; me: Me | null; refresh: () => Promise<void>; panel?: boolean }) {
  ...
  const editor = (<form onSubmit={publish} className="compact-form">
  ...
  return <section id="progress" className="detail-section progress-log">
    ...
    {me ? panel ? <><button type="button" className="np-panel-write" onClick={() => setWriting(true)}>Add an update →</button><PanelDialog title="Add a progress update" open={writing} onClose={() => setWriting(false)}>{editor}</PanelDialog></> : <details className="progress-editor"><summary>Add a progress update</summary>{editor}</details> : <a className="text-control" href={`/signin?next=${encodeURIComponent(`/c/${challenge.slug}#progress`)}`}>Sign in to add a progress update</a>}
  </section>
}
