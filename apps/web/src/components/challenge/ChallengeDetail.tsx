...
export default function ChallengeDetail({ initial, rings }: { initial?: Detail; rings: number }) {
  ...
  return <div>
    ...
    <header className="detail-header">
      ...
      <div className="detail-byline"><span className="author-avatar" aria-hidden="true">{c.author.handle.slice(0, 1).toUpperCase()}</span><span>{c.source ? 'Shared' : 'Spotted'} by <a href={`/people?handle=${encodeURIComponent(c.author.handle)}`}><strong>@{c.author.handle}</strong></a><span> · Active {ago(c.last_activity_at)}</span></span></div>
