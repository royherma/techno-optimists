...
import AiAccountPanel from './AiAccountPanel'

const tabs = [ ['activity', 'Your threads'], ['profile', 'Profile'], ['skills', 'Skills & roles'], ['compute', 'Contribute compute'], ['signin', 'Sign-in'] ] as const
export default function AccountWorkspace() {
  ...
  return <div className="account-workspace">
    ...
    <div className="account-tabs" role="tablist" aria-label="Account sections">{tabs.map(([id, label], index) => <button key={id} id={`account-tab-${id}`} type="button" role="tab" aria-selected={active === id} aria-controls={id === 'activity' ? 'account-panel-activity' : id === 'compute' ? 'account-panel-compute' : 'account-panel-settings'} tabIndex={active === id ? 0 : -1} onClick={() => select(id)} onKeyDown={(e) => {
    ...
    <div className="account-body">
      {error ? <div className="account-empty"><h2>Your account could not load</h2><p>Your connection may have dropped.</p><button onClick={() => void load()}>Try again</button></div> : person === undefined ? <p role="status">Loading your account...</p> : person === null ? <div className="account-empty"><h2>A place for your next step</h2><p>Sign in to find your marked threads and manage your profile.</p><a className="account-primary" href={`/signin?next=${encodeURIComponent(`/settings#${active}`)}`}>Sign in to your account</a></div> : <>
        ...
        <section role="tabpanel" id="account-panel-compute" aria-labelledby="account-tab-compute" hidden={active !== 'compute'} tabIndex={0}><AiAccountPanel /></section>
        <div className="form-surface" hidden={active === 'activity' || active === 'compute'} role="tabpanel" id="account-panel-settings" aria-labelledby={`account-tab-${active === 'activity' || active === 'compute' ? 'profile' : active}`} tabIndex={0}><SettingsForm person={person} section={active} /></div>
      </>}
    </div>
  </div>
}
