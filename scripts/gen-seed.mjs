...
const people = [
  ...
  ['p_mei','mei','Mei Lin Chen','Shenzhen, China',['electronics','sourcing','PCB'],['builder','expert']],
  ['p_tomas','tomas','Tomas Wojcik','Krakow, Poland',['embedded','rust','firmware'],['builder','thinker']],
  ['p_aisha','aisha','Aisha Okonkwo','Lagos, Nigeria',['public health','entomology'],['researcher','expert']],
  ['p_dave','dave','Dave Brennan','Oakland, USA',['thermodynamics','simulation'],['thinker','researcher']],
  ['p_sofia','sofia','Sofia Marchetti','Palermo, Italy',['marine biology','cold chain'],['researcher','tester']],
  ['p_arun','arun','Arun Pillai','Kochi, India',['solar','power electronics'],['builder','expert']],
  ['p_hana','hana','Hana Kobayashi','Osaka, Japan',['industrial design','CAD'],['builder','thinker']],
  ['p_lucas','lucas','Lucas Ferreira','Belem, Brazil',['drones','computer vision'],['builder','scout']],
  ['p_ingrid','ingrid','Ingrid Halvorsen','Tromso, Norway',['materials','insulation'],['expert','researcher']],
]
...
for (const [id, handle, name, location, skills, roles] of people) {
  out.push(`INSERT INTO people (id,handle,name,avatar_url,location,skills,roles) VALUES (${q(id)},${q(handle)},${q(name)},NULL,${q(location)},${j(skills)},${j(roles)});`)
}
out.push('')
