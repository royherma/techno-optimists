...
Index and Map are two views of one browsing surface. Both must render BrowseFrame (shared BrowseToolbar, content island and SheetLegend). The active view is blue in both the view links and rail - the one duplication that earns its place, because the rail is the phone's bottom navigation and the view links are not. Type and search filters apply to both map pins and the readable list. Keep page-specific map content separate from the shared frame.
