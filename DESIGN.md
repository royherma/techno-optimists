...
The shared top toolbar carries Index and Map on phones as well as desktop. Preserve safe-area spacing. The legend becomes an expandable disclosure; type filters remain in their own row. Challenge media and supporting content stack, the lifecycle wraps, and headings use the full available width. Test 390px, 1024px and reference-width desktop layouts without horizontal overflow.
...
The newspaper is the site: it is served at the bare paths (`/`, `/map`, `/c/<slug>`) and every screen follows the [editorial brief](docs/2026-09-11-editorial-v2.md) and its saved newspaper references. The masthead, ruled columns, full right-hand legend and page controls belong to reusable newspaper components. The front page uses six, three or one story per edition; details use sections, long reading uses page turns, and contribution uses explicit steps. Classic is the alternative rendering, kept under `/v1` and reachable from the same screen. Keep the impact and lifecycle guide visible on laptops and offer it as its own view on phones. On Challenge pages, highlight the current stage and explicitly mark unspecified impact. Participation controls belong in How you can help; do not repeat them as a non-interactive action legend.

Newspaper Challenge sections stay visible together in a newspaper grid. Do not replace them with tabs or collapsible sections. Longer content turns pages inside its own panel. Impact rings use the original legend tiers and must not derive from reaction or update counts; until impact is explicitly stored, show it as unspecified.

Source and place share a horizontal panel, with independently paged source text and a local map centered on the reported coordinates. Use readable local cartography and visible OpenStreetMap attribution; the world atlas belongs on global browsing surfaces.
